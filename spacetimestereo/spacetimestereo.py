...
class SpacetimeStereo:
    ...
    @staticmethod
    def calc_disparity(imgs1, imgs2, disp_range, filter_size=3, device='', batches=10):
        ...
        avr = torch.nn.AvgPool2d(filter_size, stride=1, padding=1)

        # calc cost for different disparity values
        y_step= math.ceil(h/batches)
        for y in range(0, h, y_step):
            y_max = min(y+y_step, h)
            imgs1t = torch.tensor(imgs1[:,:,y:y_max,:], dtype=torch.float16).to(device)
            imgs2t = torch.tensor(imgs2[:,:,y:y_max,:], dtype=torch.float16).to(device)
        ...
        #     filtert         = torch.tensor(np.ones((1,1,filter_size,filter_size), dtype=np.float32)/(filter_size*filter_size), dtype=torch.float16, device=device)
    ...
    @staticmethod
    def calc_shadow_map_from_bw(black_img, white_img, shadow_th):

        shadow_map = calc_shadow_map(black_img, white_img, shadow_th)
        shadow_map = cv2.medianBlur(shadow_map, 5)

        return shadow_map

    @staticmethod
    def calc_shadow_map_from_array(imgs, shadow_th):
        black_img = np.array(imgs).min(axis=0)
        white_img = np.array(imgs).max(axis=0)

        shadow_map = calc_shadow_map(black_img, white_img, shadow_th)
        shadow_map = cv2.medianBlur(shadow_map, 5)

        return shadow_map, white_img, black_img

    def disparity_to_point_cloud(self, disparity_map, texture_img, z_lim = None):
        # transform disparity map to point cloud
        image_size = texture_img.shape[1],  texture_img.shape[0]
        # image_size = texture_img.shape[0],  texture_img.shape[1]
        Q, Q2  = create_reporjection_matrix(self.cam_params, image_size)
        xyz, rgb = disparity_to_point_cloud(disparity_map, texture_img, Q2)

        if z_lim != None:
            xyz, rgb = remove_points(xyz, rgb, z_lim)

        pcd = create_point_cloud_open3d(xyz, rgb)
